...
test("app renders Movie from API", async () => {
    ...
    await waitFor(() => expect(getByTestId(/name/i)).toHaveTextContent(/Stranger Things/i));
});
...
test('Episodes are rendered', async () => {
    ...
    await waitFor(() => {
        ...
        async () => {
            await waitFor(() => fireEvent.click(getByText(/Season 3/i)));
            async () => await waitFor(expect(getByTestId(/epi-name/i)).toHaveTextContent(/Chapter One: Suzie, Do You Copy?/i));
        }
